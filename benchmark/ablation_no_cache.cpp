...
int main(int argc, char *argv[]) {
    ...
    #ifdef NO_MEMORY_CACHE
        delay = get_delay();
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    // #ifdef NO_MEMORY_CACHE
    //     delay = get_delay();
    //     LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    // #endif
    results["where"]["Stkcd"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["where"]["Clsprc"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["apply"]["Stkcd"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["apply"]["Opnprc"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["sort"]["Stkcd"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["sort"]["Clsprc"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["sort"]["Hiprc"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["join"]["Stkcd_0_20_vs_20_40"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["groupby"]["Stkcd_Clsprc"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["interval"]["30D"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["interval"]["90D"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    stream << synchronize(); 
        ...
        LUISA_INFO("delay in {} ms", delay  / static_cast<double>(test_round));
    #endif
    results["interval"]["365D"] = clock.toc() / static_cast<double>(test_round) + delay / static_cast<double>(test_round);
    ...
    // 写入文件
    std::ofstream file("./results/ablation_no_cache.json");
    if (file.is_open()) {
        file << json_ss.str();
        file.close();
        std::cout << "Benchmark results saved to 'ablation_no_cache.json'\n";
    } else {
        std::cerr << "Failed to open file 'ablation_no_cache.json'\n";
    }
