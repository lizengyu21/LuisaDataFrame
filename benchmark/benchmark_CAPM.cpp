...
int main(int argc, char *argv[]) {
    ...
    table.dropna("PrevClsprc");
    table.print_table_length();
    table._group_by("Stkcd", {{"PrevClsprc", {AggeragateOp::MEAN}}});
    table.print_table();
    ...
    // int test_round = std::stoi(argv[2]);
    ...
    //     auto filtered_table = table.where("DailyReturn", res, FilterOp::GREATER, 0.0f);
    //     auto agg_table = filtered_table.group_by("Stkcd", agg_op_map);
    //     agg_table._sort("MEAN(DailyReturn)", SortOrder::Descending);
    ...
    // LUISA_INFO("DailyReturn in {} ms", clock.toc());
    ...
    //     auto filtered_table = week_data.where("WeeklyReturn", res, FilterOp::GREATER, 0.0f);
    //     auto agg_table = filtered_table.group_by("Stkcd", agg_op_map);
    //     agg_table._sort("MEAN(WeeklyReturn)", SortOrder::Descending);
    //     // agg_table.print_table();
    // }
    // stream << synchronize();
    // results["Result"]["WeeklyReturn"] = clock.toc() / static_cast<double>(test_round);
    // LUISA_INFO("WeeklyReturn in {} ms", clock.toc());



    // agg_op_map.clear();
    // agg_op_map.insert({"MonthlyReturn", {AggeragateOp::MEAN, AggeragateOp::MAX, AggeragateOp::COUNT}});
    // clock.tic();
    // for (int i = 0; i < test_round; ++i) {
    //     auto week_data = table.interval("Stkcd", "", 60*60*24*30, {AggeragateOp::MEAN});
    //     auto res = week_data.apply("MEAN(Clsprc)", "MEAN(PrevClsprc)", apply_func);
    //     // week_data["WeeklyReturn"] = std::move(res);
    //     auto filtered_table = week_data.where("MonthlyReturn", res, FilterOp::GREATER, 0.0f);
    //     auto agg_table = filtered_table.group_by("Stkcd", agg_op_map);
    //     agg_table._sort("MEAN(MonthlyReturn)", SortOrder::Descending);
    //     // agg_table.print_table();
    // }
    // stream << synchronize();
    // results["Result"]["MonthlyReturn"] = clock.toc() / static_cast<double>(test_round);
    // LUISA_INFO("MonthlyReturn in {} ms", clock.toc());



    // agg_op_map.clear();
    // agg_op_map.insert({"YearlyReturn", {AggeragateOp::MEAN, AggeragateOp::MAX, AggeragateOp::COUNT}});
    // clock.tic();
    // for (int i = 0; i < test_round; ++i) {
    //     auto week_data = table.interval("Stkcd", "", 60*60*24*365, {AggeragateOp::MEAN});
    //     auto res = week_data.apply("MEAN(Clsprc)", "MEAN(PrevClsprc)", apply_func);
    //     // week_data["WeeklyReturn"] = std::move(res);
    //     auto filtered_table = week_data.where("YearlyReturn", res, FilterOp::GREATER, 0.0f);
    //     auto agg_table = filtered_table.group_by("Stkcd", agg_op_map);
    //     agg_table._sort("MEAN(YearlyReturn)", SortOrder::Descending);
    //     // agg_table.print_table();
    // }
    // stream << synchronize();
    // results["Result"]["YearlyReturn"] = clock.toc() / static_cast<double>(test_round);
    // LUISA_INFO("YearlyReturn in {} ms", clock.toc());
    ...
    // // 写入文件
    // std::string json_filename = "./results/ours_real_benchmark_results.json";
